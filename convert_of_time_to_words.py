...
class HourToTextConverter:
    def __init__(self):
        # Lista y diccionario que contienen las palabras que se usarán para crear la hora en texto
    ...
    # if para revisar, de acuerdo a los minutos, cómo se va a estructurar el texto final
    def election(self, minute, hour): # Si ingresa 00:59 o 12:59 imprime directo one minute to one
        if ((hour == 12) or (hour == 0)) and (minute == 59):
            ...
            if 60 - minute == 1 or 60 - minute == 59:
                self.format_0(word=self.times[1] if minute == 1 else self.times[4],
                              hour=12 if hour == 0 else hour + 1)
            else:
                while True: #Bucle hasta que encuentre los valores correctos
                    if 2 <= minute <= 14 or 16 <= minute <= 29:
                        word = self.times[1]
                        hour = self.numbers[hour] if not hour == 0 else self.numbers[12]
                        minute = self.numbers[minute]
                        self.format_1(minute=minute, word=word, hour=hour)
                        break
                    elif minute == 15 or minute == 45 or minute == 30: #
                        self.format_2(
                            word_1=self.times[2] if (minute == 15) or (minute == 45)
                            else self.times[3],
                            word_2=self.times[4] if (minute == 45) else self.times[1],
                            hour=self.numbers[hour + 1] if not (minute == 15) or (minute == 30)
                            else self.numbers[hour])
                        break
                    elif 31 <= minute <= 44 or 46 <= minute <= 58:  # minute + "minutes" + to + hour soon
                        word = self.times[4]
                        ...
                        hour = self.numbers[hour] if not hour == 0 else self.numbers[12]
                        print(hour + " " + self.times[minute])  # se imprime hour + o' clock si minute = 0
                        break
...
# Crear una instancia de la clase y llamar al método para convertir la hora a texto
